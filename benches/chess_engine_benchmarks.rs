...
use yrjo_chess_engine::alpha_beta_algorithm::{AlphaBetaAlgorithm, AlgorithmTraits};
...
fn bench_evaluation(c: &mut Criterion) {
    ...
    for (name, board) in positions.iter() {
        group.bench_with_input(BenchmarkId::new("default_eval", name), board, |b, board| {
...
fn bench_complete_analysis(c: &mut Criterion) {
    ...
    let positions = [
        ("tactical_position", BenchPositions::tactical_position()),
        ("endgame_position", BenchPositions::endgame_position()),
    ];
