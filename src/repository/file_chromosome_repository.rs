...
impl FileChromosomeRepository {
    pub fn new<P: AsRef<Path>>(file_path: P) -> Result<Self, String> {
        ...
        let chromosomes_dir = original_path.parent().unwrap_or(Path::new(".")).join("chromosomes");

        // Create the chromosomes directory if it doesn't exist
        fs::create_dir_all(&chromosomes_dir).map_err(|e| format!("Failed to create chromosomes directory: {e}"))?;

        // Place the file inside the chromosomes folder
        let file_name = original_path.file_name().unwrap_or(std::ffi::OsStr::new("chromosomes.json"));
    ...
    fn read_chromosomes_from_file(file_path: &Path) -> Result<Vec<Chromosome>, String> {
        ...
        let history: TournamentHistory = serde_json::from_str(&content).map_err(|e| format!("Failed to parse tournament history JSON: {e}"))?;
    ...
    fn read_tournament_history_from_file(file_path: &Path) -> Result<TournamentHistory, String> {
        ...
        serde_json::from_str(&content).map_err(|e| format!("Failed to parse tournament history JSON: {e}"))
    }
    ...
    fn write_tournament_history_to_file(file_path: &Path, history: &TournamentHistory) -> Result<(), String> {
        ...
        {
            let json = serde_json::to_string_pretty(history).map_err(|e| format!("Failed to serialize tournament history: {e}"))?;

            let mut temp_file = fs::File::create(&temp_path).map_err(|e| format!("Failed to create temporary file: {e}"))?;

            temp_file.write_all(json.as_bytes()).map_err(|e| format!("Failed to write to temporary file: {e}"))?;

            temp_file.sync_all().map_err(|e| format!("Failed to sync temporary file: {e}"))?;
        }
...
impl ChromosomeRepository for FileChromosomeRepository {
    fn read_chromosomes(&self) -> Result<Vec<Chromosome>, String> {
        ...
        let _read_lock = self.file_lock.read().map_err(|_| "Failed to acquire read lock for reading chromosomes")?;
    ...
    fn write_chromosomes(&mut self, chromosomes: &[Chromosome]) -> Result<(), String> {
        ...
        let _write_lock = self.file_lock.write().map_err(|_| "Failed to acquire write lock for writing chromosomes")?;
    ...
    fn write_tournament_winners_with_count(&mut self, winners: &[Chromosome], player_count: i32) -> Result<(), String> {
        // Use write lock - exclusive access for writing
        let _write_lock = self.file_lock.write().map_err(|_| "Failed to acquire write lock for writing tournament winners")?;
    ...
    fn validate_player_count(&self, player_count: i32) -> Result<(), String> {
        let _read_lock = self.file_lock.read().map_err(|_| "Failed to acquire read lock for validation")?;
